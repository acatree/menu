...
from .latex_utils import finalize_latex_output   # ✅ 추가
...
def generate_paper(topic, authors=None, affiliations=None, emails=None, api_key=None):
    ...
    doc.append(NoEscape(r'\tableofcontents'))   # ✅ 목차 추가
    ...
    abstract_text = finalize_latex_output(text_utils.clean_section_text(abstract_text))  # ✅ 후처리
    ...
    # 5. 섹션 작성
    section_requirements = {"서론": 300, "관련 연구": 350, "연구 방법": 300, "분석": 600, "결론": 250}
    sections = list(section_requirements.keys())

    for sec in sections:
        doc.append(NoEscape(f"\\section{{{sec}}}"))
        min_words = section_requirements[sec]
        ...
        if sec == "분석":
            ...
            data_code = openai_utils.ask_question(
                f"'{creative_title}' 관련 통계 데이터 예시 생성. pandas DataFrame(df) 5~6개 열, 20~30행 포함.",
                api_key=api_key
            )
            ...
            text_exp = openai_utils.ask_question(
                f"'{creative_title}' '{sec}' 섹션 작성. df 기반 해석 포함, 최소 {min_words}단어 이상.",
                api_key=api_key
            )

            text = finalize_latex_output(text_utils.clean_section_text(text_exp))  # ✅ 후처리
            ...
            try:
                table_latex = df.to_latex(index=False, caption=f"{sec} 관련 표", label=f"tab:{sec}")
            ...
            text = finalize_latex_output(text)  # ✅ 후처리
            doc.append(NoEscape(text))
